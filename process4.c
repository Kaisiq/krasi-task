...
int main(int argc, char *argv[]) {
    ...
    printf("%s%s[Process 4 - PID: %d] Started. Delay: %d ms. Reading strings -> Socket (%s)%s\n",
    ...
    while (connect(sock_fd, (struct sockaddr*)&server_addr, sizeof(server_addr)) < 0) {
        if (errno == ENOENT || errno == ECONNREFUSED) { // Socket file doesn't exist or server not listening
            ...
            close(sock_fd);
             printf("%s%s[Process 4 - PID: %d] Terminating due to Socket connection error.%s\n", fg_code, bg_code, my_pid, COLOR_RESET);
            return EXIT_FAILURE;
        }
    }
    ...
    printf("%s%s[Process 4 - PID: %d] Enter strings (Ctrl+D or signal to stop):\n%s", fg_code, bg_code, my_pid, COLOR_RESET);
    fflush(stdout);
    ...
    while (!terminate_flag) {
        ...
        if (fgets(input_string, sizeof(input_string), stdin) != NULL) {
            ...
            // Check if string is empty after removing newline (user just pressed Enter)
             if (strlen(input_string) == 0 && !terminate_flag) {
                // Maybe prompt again or handle as needed. Here, we just continue.
                continue;
             }
            ...
            if (bytes_sent < 0) {
                if (errno == EPIPE) { // Server (P5) closed connection
                     printf("\n%s%s[Process 4 - PID: %d] Detected P5 closed the socket connection. Terminating.%s\n", fg_code, bg_code, my_pid, COLOR_RESET);
                     terminate_flag = 1; // Stop loop
                } else {
                    perror("Process 4: Failed to send data");
                    // Decide how to handle other errors (maybe terminate)
                    terminate_flag = 1;
                }
            } else if (bytes_sent < sizeof(msg)) {
                fprintf(stderr, "%s%s[Process 4 - PID: %d] Warning: Partial send on socket.%s\n", fg_code, bg_code, my_pid, COLOR_RESET);
            } else {
                 printf("%s%s[Process 4 - PID: %d] Sent: \"%s\"%s\n", fg_code, bg_code, my_pid, msg.value, COLOR_RESET);
                 fflush(stdout);
            }
        } else {
            // fgets failed (e.g., EOF)
             if (feof(stdin)) {
                printf("\n%s%s[Process 4 - PID: %d] EOF detected on input. Terminating.%s\n", fg_code, bg_code, my_pid, COLOR_RESET);
             } else {
